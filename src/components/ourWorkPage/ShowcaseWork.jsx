    ...
    <div className='ow-showcase-work-container mb-32'>
        ...
        <ul className='grid grid-flow-col grid-cols-3 grid-rows-2 gap-6 py-5'>
      {
        SampleDocumentariesList.map(documentary => {
          return(
            <li className='m-3'>
              <img src={documentary.image} alt="doc pic" className='h-44'/>
              <h2 className='text-lg font-md pt-4'>{documentary.title}</h2>
              <p className='text-xs italic font-light'>{documentary.year} - {documentary.lenght} minutes</p>
              <p  className='text-md font-light w-80'>{documentary.caption}</p>
            </li>
          )
        })
      }
